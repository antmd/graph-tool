...
{
    get_combined_degree_histogram(DegreeSelector1& deg1, DegreeSelector2& deg2):
    ...
    void operator()(Graph &g, Hist &hist) const
    {
        ...
        for (i = 0; i < N; ++i)
        {
            ...
            s_hist[make_pair(first_type(_deg1(v,g)),
                             second_type(_deg2(v,g)))]++;
        }
...
struct choose_combined_degree_histogram
{
    ...
    {
        check_second_degree
            (choose_combined_degree_histogram<SecondDegreeSelectors> &parent):
                _parent(parent) {}
...
GraphInterface::GetCombinedVertexHistogram(deg_t deg1, deg_t deg2) const
{
    ...
    try
    {
        mpl::for_each<degree_selectors>
            (choose_combined_degree_histogram<degree_selectors>(*this, deg1,
                                                                deg2, hist));
    }
...
{
    get_average_combined_degree_correlation(DegreeSelector1& deg1,
                                            DegreeSelector2& deg2):
        _deg1(deg1), _deg2(deg2) {}
...
const
{
    ...
    {
        mpl::for_each<degree_selectors>
            (choose_average_combined_degree_correlation<degree_selectors>
             (*this, deg1, deg2, avg_corr));
    }
    catch (dynamic_get_failure &e)
    {
        throw GraphException("error getting scalar property: " +
